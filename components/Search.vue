      ...
      <ais-autocomplete v-click-outside="onClickOutside">
        <div slot-scope="{ currentRefinement, indices, refine }">
          ...
          <input
            ...
            class="px-5 py-2 rounded bg-gray-50 w-64 border-none focus:w-72 transition-all duration-500"/>
            ...
            <div class="rounded-md shadow-lg overflow-hidden">
              <div class="relative grid gap-6 bg-gray-900 text-gray-100 px-5 py-6 sm:gap-8 sm:p-8">
                <div v-if="currentRefinement" v-for="section in indices" :key="section.objectID" class="divide-y divide-blue-900">
                  <!-- Link -->
                  <nuxt-link to="#" v-for="(hit, index) in section.hits" :key="hit.objectID" class="block text-sm col-span-2 py-2 transition ease-in-out duration-150" :class="{ 'bg-blue-900': isCurrentIndex(index) }">
  ...
  data() {
    return {
      searchClient: algoliasearch(
        "20P06SW2J9",
        "1b504947a7f7df96fa0f978ecec060f4"
      ),
      showResults: false,
      highlightedIndex: -1,
    };
  ...
  computed: {
    searchPlaceholder() {
      if (navigator.appVersion.indexOf("Mac") !== -1) {
        return "Type - ⌘k to focus";
      } else if (navigator.appVersion.indexOf("Win") !== -1) {
        return "Type - Win + k to focus..";
      } else {
        return "Search";
      }
    },
